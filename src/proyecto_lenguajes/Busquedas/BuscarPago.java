...
public class BuscarPago extends javax.swing.JFrame {
    ...
    public void buscar() {
        ...
        String id = JOptionPane.showInputDialog(null, "Digite la cedula del empleado para buscar los pagos: ");
        try {
            while (!(id.length() == 9)) {
            ...
            if (existe(id)) {
                ...
                try {
                    if (!(s.equals(""))) {
                        opcion = Integer.parseInt(JOptionPane.showInputDialog(null, s + "Digite el numero de pago que desea consultar: "));
                        try {
                            conectar();
                            Statement st = con.createStatement();
                            ResultSet rs = st.executeQuery("select * from pago_salario where num_pago=" + opcion);
                            rs.next();
                            p.setNum_pago(rs.getInt(1));
                            p.setFecha(dateFormat.format(rs.getDate(2)));
                            p.setDescripcion(rs.getString(3));
                            p.setId_empleado(rs.getString(4));
    ...
    public void llenar(Pago p) {
        String nombre = "";
        jSpinner1.setValue(p.getNum_pago());
        jTextField8.setText(p.getFecha());
        jTextField7.setText(p.getId_empleado());
        jTextArea1.setText(p.getDescripcion());
        jTextField3.setText(String.valueOf(p.getTotal()));
        try {
            conectar();
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("select nombre,apellidos from empleado where id_empleado='" + p.getId_empleado() + "'");
            rs.next();
